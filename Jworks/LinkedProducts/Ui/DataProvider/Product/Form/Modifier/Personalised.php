 ...
 */
namespace Jworks\LinkedProducts\Ui\DataProvider\Product\Form\Modifier;
 ...
 * @since 101.0.0
 */
class Personalised extends \Magento\Catalog\Ui\DataProvider\Product\Form\Modifier\AbstractModifier
{
    const DATA_SCOPE = '';
    const DATA_SCOPE_PERSONALISED = 'personalised';
    const GROUP_RELATED = 'personalised';
    /**
     * @var string
     */
    ...
    public function modifyMeta(array $meta)
    {
        $meta = array_replace_recursive(
            $meta,
            [
                static::DATA_SCOPE_PERSONALISED => [
                    'children' => [
                        $this->scopePrefix . static::DATA_SCOPE_PERSONALISED => $this->getRelatedFieldset()
                    ],
                    'arguments' => [
                        'data' => [
                            'config' => [
                                'label' => __('Personalised Products'),
                                'collapsible' => true,
    ...
    {
        $content = __(
            'Personalised products are shown to customers for creating Personalised Features'
        );
        ...
        return [
            'children' => [
                'button_set' => $this->getButtonSet(
                    $content,
                    __('Add Personalised Products'),
                    $this->scopePrefix . static::DATA_SCOPE_PERSONALISED
                ),
                'modal' => $this->getGenericModal(
                    __('Add Personalised Products'),
                    $this->scopePrefix . static::DATA_SCOPE_PERSONALISED
                ),
                static::DATA_SCOPE_PERSONALISED => $this->getGrid($this->scopePrefix . static::DATA_SCOPE_PERSONALISED),
            ],
            'arguments' => [
                'data' => [
                    'config' => [
